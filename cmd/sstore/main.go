...
func (a App) handleStoreNotify(ws *websocket.Conn, wg *sync.WaitGroup) {
	...
	ch := make(chan sstore.OpInfo)
	a.store.Notify(ch, sstore.OperationDelete, sstore.OperationInsert, sstore.OperationUpdate)
	for {
...
func (a App) handleClient(ws *websocket.Conn, wg *sync.WaitGroup) {
	...
	for {
		...
		case opTypeSet:
			if _, err := a.store.Set(data.Value); err != nil {
