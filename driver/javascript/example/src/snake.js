...
function SnakeJS(parentElement, config) {
    ...
    var config = config
        ? utilities.mergeObjects(defaultConfig, config)
        : defaultConfig;
    ...
    function Engine(parentElement) {
        ...
        var gameOver = function() {
            ...
            var removeTail = function() {
                ...
                } else setTimeout(resurrect, config.frameInterval * 10);
            };
        ...
        var moveSnake = function(desiredDirection) {
            ...
            var newDirection = actualDirection(
                desiredDirection || constants.DEFAULT_DIRECTION
            );

            var newHead = movePoint(head, newDirection);

            if (!insideGrid(newHead, grid)) shiftPointIntoGrid(newHead, grid);
            ...
            if (snake.growthLeft >= 1) snake.growthLeft--;
            else snake.points.pop();
        ...
        var randomCandy = function() {
            ...
            if (probabilitySeed < 0.75) var newType = constants.CANDY_REGULAR;
            else if (probabilitySeed < 0.95)
                var newType = constants.CANDY_MASSIVE;
            else var newType = constants.CANDY_SHRINKING;
        ...
        var actualDirection = function(desiredDirection) {
            if (snake.points.length === 1) return desiredDirection;
            else if (
                utilities.oppositeDirections(snake.direction, desiredDirection)
            ) {
        ...
        var insideGrid = function(point, grid) {
            if (
                point.left < 0 ||
                point.top < 0 ||
                point.left >= grid.width ||
                point.top >= grid.height
            ) {
                return false;
            } else {
                return true;
    ...
    function Snake() {
        ...
        this.collidesWith = function(point, simulateMovement) {
            ...
            else range = this.points.length;
            for (var i = 0; i < range; i++) {
                if (point.collidesWith(this.points[i])) return true;
            }
            return false;
        };
    }
    ...
    function Candy(point, type) {
        (this.point = point), (this.type = type), this.score, this // Increment in score when eaten by snake
            .calories, this.radius, this.color, this.decrement, this.minRadius; // How much growth the snake gains if it eats this candy // Radius of the candy, relative to config.pointSize // Color of the candy // If greater than 0, the radius of the candy will shrink... // until it reaches this minimum value. Then it will disappear
    ...
    function Utilities() {
        ...
        this.oppositeDirections = function(direction1, direction2) {
            ...
            if (
                Math.abs(direction1) == Math.abs(direction2) &&
    ...
    function View(parentElement, backgroundColor) {
            ...
            if (snake.points.length === 1) {
                ...
                for (var i = 0; i < snake.points.length; i++) {
                    ...
                    if (i === 0) {
                        ...
                        var currentPointPosition = getPointPivotPosition(
                            currentPoint
                        );
                        // Don't draw anything, just move the "pencil" to the position of the head
                        ctx.moveTo(
                            currentPointPosition.left,
                            currentPointPosition.top
                        );
                    } else {
                        ...
                        if (
                            Math.abs(prevPoint.left - currentPoint.left) <= 1 &&
                            Math.abs(prevPoint.top - currentPoint.top) <= 1
                        ) {
                            // The position of this point in screen pixels
                            var currentPointPosition = getPointPivotPosition(
                                currentPoint
        ...
        this.drawScore = function(score, highScore) {
            ...
            ctx.fillText(
                score,
                config.gridWidth * config.pointSize - horizontalMargin,
                constants.SCOREBOARD_HEIGHT - bottomMargin
            );
        ...
        var drawEye = function(snake) {
            ...
            if (snake.alive) {
                ...
                ctx.lineWidth = 2;
                ctx.moveTo(
                    headPosition.left - length(0.1),
                    headPosition.top - length(0.1)
                );
                ctx.lineTo(
                    headPosition.left + length(0.1),
                    headPosition.top + length(0.1)
        ...
        var getPointPivotPosition = function(point) {
            var position = {
                left: point.left * length(1) + length(0.5),
                top: point.top * length(1) + length(0.5)
            };
            return position;
        };
